...
public class ItemBuilder {
	...
	public ItemBuilder(ItemStack stack) {
		this(stack.getType(), stack.getAmount(), stack.getDurability(), stack.getEnchantments(), stack.getItemMeta().getDisplayName(), stack.getItemMeta().getLore(), new ArrayList<ItemFlag>(stack.getItemMeta().getItemFlags()));
		this.stack = stack;
	}
	
	public ItemBuilder(Material type, int amount) {
		this(type, amount, (short)0, new HashMap<Enchantment, Integer>(), null, new ArrayList<String>(), new ArrayList<ItemFlag>());
	}
	
	public ItemBuilder(Material type, int amount, short data, Map<Enchantment, Integer> enchants, String display, List<String> lore, List<ItemFlag> flags) {
		this.type = type;
		this.amount = amount;
		this.data = data;
	...
	public ItemBuilder addEnchantment(Enchantment enchant, int level, boolean bypass) {
		enchants.put(enchant, bypass ? level : Math.max(Math.min(level, enchant.getMaxLevel()), enchant.getStartLevel()));
		return this;
	}
	...
	public ItemStack createItem() {
		...
		if (display != null)
			meta.setDisplayName(StringUtilities.convertColorCodes(display));
		meta.setLore(lore.stream().map(e -> StringUtilities.convertColorCodes(e)).collect(Collectors.toList()));
